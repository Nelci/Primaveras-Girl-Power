    ...
    public class MAttackTrigger : MDamager
    {
        ...
        private void FindTrigger()
        {
            ...
            if (Trigger)
            {
                ...
                Trigger.gameObject.SetLayer(2, false); //Force [Ignore Raycast] on the Attack Trigger

                Proxy = Trigger.GetComponent<TriggerProxy>();

                if (Proxy == null)
                {
                    Proxy = Trigger.gameObject.AddComponent<TriggerProxy>();            //Create a proxy to comunicate the Collision and trigger events in case the melee  
                    Proxy.hideFlags = HideFlags.HideInInspector; //Hide the Proxy in the inspector
                    Proxy.SetLayer(Layer, TriggerInteraction, Owner.transform);
                }
                else
                {
                    Proxy.Layer = Proxy.Layer | Layer; //combine both layers in case is already created
                }
