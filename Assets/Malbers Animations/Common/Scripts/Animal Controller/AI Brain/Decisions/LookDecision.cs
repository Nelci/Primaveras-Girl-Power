...
namespace MalbersAnimations.Controller.AI
{
    ...
    public class LookDecision : MAIDecision
    {
       ...
       // [Utilities.Flag]
        ...
        public override void PrepareDecision(MAnimalBrain brain, int index)
        {
            ...
            StoreRenderers(brain, index);
        }

        /// <summary> Store all renderers found on the GameObjects  </summary>
        private static void StoreRenderers(MAnimalBrain brain, int index)
        {
            if (brain.DecisionsVars[index].gameobjects != null && brain.DecisionsVars[index].gameobjects.Length > 0)
            {
                brain.DecisionsVars[index].Components = new Component[brain.DecisionsVars[index].gameobjects.Length];

                for (int i = 0; i < brain.DecisionsVars[index].gameobjects.Length; i++)
                {
                    brain.DecisionsVars[index].Components[i] = brain.DecisionsVars[index].gameobjects[i].GetComponentInChildren<Renderer>(); //Store the Renderer
                }
            }
        }
        ...
        private bool IsInFieldOfView(MAnimalBrain brain, Vector3 Center, out float Distance)
        {
            ...
            Distance = Vector3.Distance(Center, brain.Eyes.position) * 0.9f; //Important, otherwise it will find the ground for Objects to close to it

            if (LookAngle == 0 || LookRange <= 0) return true; //Means the Field of view can be ignored

            if (Distance < LookRange) //Check if whe are inside the Look Radius
            {
        ...
        {
            var All = brain.DecisionsVars[index].gameobjects; //catch all the saved gameobjects with a tag
            ...
            for (int i = 0; i < All.Length; i++)
            {
                if (All[i] != null)
                {
                    var Center = All[i].transform.position + new Vector3(0, brain.Animal.Height, 0);

                    var renderer = brain.DecisionsVars[index].Components[i];
    ...
    public class LookDecisionEditor : Editor
    {
        ...
        SerializedProperty
            Description, UnityTag, debugColor, zoneType, ZoneID, tags, LookRange, LookAngle, lookFor, transform, gameobject, gameObjectSet, AllZones,
