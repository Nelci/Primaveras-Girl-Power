...
namespace MalbersAnimations.Controller
{
    public partial class MAnimal
    {
        void Awake()
        {
            ...
            if (Rotator != null)
            {
                ...
                { 
                    ...
                    if (RootBone == null) Debug.LogWarning("Make sure the Root Bone is Set on the Advanced Tab -> Misc -> RootBone. This is the Character's Avatar root bone");
                }
        ...
        public virtual void ResetController()
        {
            ...
            {
                Debug.LogWarning("Animal Controller require an Animator Controller.");
                enabled = false;
            }
       ...
       //     Debug.DrawRay(transform.position, move * 5, Color.yellow);
            ...
            {
                DeltaAngle = 0f; //Remove the NAN Bug
            }
            ...
            foreach (var trySt in states)
            {
                ...
                if (!ActiveState.IsPending && ActiveState.CanExit)    //Means a new state can be activated
                {
                    if ( trySt.Active && 
                        ...
                        !trySt.IsSleepFromState && 
                        !trySt.IsSleepFromMode && 
                        !trySt.IsSleepFromStance && 
                        !trySt.OnQueue && 
                         trySt.TryActivate())
                    {
                        trySt.Activate();
        ...
        {
            PitchDirection = Vector3.Lerp(PitchDirection, Move_Direction, DeltaTime * CurrentSpeedSet.PitchLerpOn * 2);
        }
        ...
        public void CalculateTargetSpeed()
        {
            ...
            TargetSpeed *= ScaleFactor;   //Apply Scale Factor
        ...
        public virtual void FreeMovementRotator(float Ylimit, float bank)
        {
            ...
            PitchAngle = Mathf.Lerp(PitchAngle, NewAngle * SmoothZYInput, DeltaTime * CurrentSpeedSet.PitchLerpOn);
