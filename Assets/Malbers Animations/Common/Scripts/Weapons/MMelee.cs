    ...
    public class MMelee : MWeapon 
    {
        ...
        private void FindTrigger()
        {
            ...
            if (meleeTrigger)
            {
                ...
                proxy = meleeTrigger.GetComponent<TriggerProxy>();

                if (proxy == null)
                {
                    proxy = meleeTrigger.gameObject.AddComponent<TriggerProxy>();            //Create a proxy to comunicate the Collision and trigger events in case the melee  
                    proxy.hideFlags = HideFlags.HideInInspector; //Hide the Proxy
                }
