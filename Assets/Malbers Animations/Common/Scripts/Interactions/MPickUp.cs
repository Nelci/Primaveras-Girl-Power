    ...
    public class MPickUp : MonoBehaviour, IAnimatorListener
    {
        ...
        private void CheckTriggerProxy()
        {
            if (PickUpArea)
            {
                if (Proxy == null)
                {
                    Proxy = PickUpArea.GetComponent<TriggerProxy>();

                    if (Proxy == null)
                    {
                        Proxy = PickUpArea.gameObject.AddComponent<TriggerProxy>();
                        Proxy.SetLayer(Layer, triggerInteraction, transform.root, Tags);
                        Proxy.hideFlags = HideFlags.HideInInspector;
                    }
                }

                PickUpArea.gameObject.SetLayer(2, false); //Force the Trigger Area to be on the Ingore Raycast Layer
