    ...
    public class MInteractor : UnityUtils, IInteractor
    {
        ...
        private void CheckTriggerProxy()
        {
            if (InteractionArea != null)
            {
                if (Proxy == null)
                {
                    Proxy = InteractionArea.GetComponent<TriggerProxy>();

                    if (Proxy == null)
                    {
                        Proxy = InteractionArea.gameObject.AddComponent<TriggerProxy>();
                        Proxy.SetLayer(Layer, TriggerInteraction, transform.root);
                ...
                InteractionArea.gameObject.SetLayer(2, false); //Force the Trigger Area to be on the Ignore Raycast Layer
                InteractionArea.isTrigger = true;
            }
            //else
            //{
            //    Debug.LogWarning($"Interactor [{name}] is missing a Trigger. Please set a Pick up Area",this);
            //}
        }
