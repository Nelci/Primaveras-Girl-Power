...
public class BasicFixedBehaviour : MonoBehaviour
{
    ...
    public bool hasFixXConstraint = false;
    ...
    private ThirdPersonFixedCamBasic camScript;           // Reference to the third person camera script.
    ...
    private List<GenericFixedBehaviour> behaviours;            // The list containing all the enabled player behaviours.
    private List<GenericFixedBehaviour> overridingBehaviours;  // List of current overriding behaviours.
    ...
    public ThirdPersonFixedCamBasic GetCamScript { get { return camScript; } }
    ...
    void Awake()
    {
        ...
        behaviours = new List<GenericFixedBehaviour>();
        overridingBehaviours = new List<GenericFixedBehaviour>();
        anim = GetComponent<Animator>();
        hFloat = Animator.StringToHash("H");
        vFloat = Animator.StringToHash("V");
        camScript = playerCamera.GetComponent<ThirdPersonFixedCamBasic>();
    ...
    void Update()
    {
        ...
        h = (joystick != null) ? joystick.Horizontal*10 : Input.GetAxis("Horizontal");
        v = (joystick != null) ? joystick.Vertical*10 : Input.GetAxis("Vertical");
    ...
    void FixedUpdate()
    {
        ...
        {
            foreach (GenericFixedBehaviour behaviour in behaviours)
            {
        ...
        {
            foreach (GenericFixedBehaviour behaviour in overridingBehaviours)
            {
                behaviour.LocalFixedUpdate();
            }
        }
        ...
        rBody.constraints = hasFixXConstraint ? 
			(RigidbodyConstraints.FreezePositionX | rBody.constraints) : 
			rBody.constraints;
    }
    ...
    public void SubscribeBehaviour(GenericFixedBehaviour behaviour)
    {
    ...
    public bool OverrideWithBehaviour(GenericFixedBehaviour behaviour)
    {
        ...
        if (!overridingBehaviours.Contains(behaviour))
        {
            ...
            if (overridingBehaviours.Count == 0)
            {
                ...
                foreach (GenericFixedBehaviour overriddenBehaviour in behaviours)
                {
    ...
    public bool RevokeOverridingBehaviour(GenericFixedBehaviour behaviour)
    {
        if (overridingBehaviours.Contains(behaviour))
        {
    ...
    public bool IsOverriding(GenericFixedBehaviour behaviour = null)
    {
        if (behaviour == null)
...
public abstract class GenericFixedBehaviour : MonoBehaviour
{
    //protected Animator anim;                       // Reference to the Animator component.
    protected int speedFloat;                      // Speed parameter on the Animator.
    protected BasicFixedBehaviour behaviourManager;     // Reference to the basic behaviour manager.
    ...
    {
        // Set up the references.
        behaviourManager = GetComponent<BasicFixedBehaviour>();
